...
def selectJ(i,oS,Ei): #内循环中的启发式算法
        maxK=-1;maxDeltaE=0;Ej=0
        oS.eCache[i]=[1,Ei]
        validEcacheList=nonzero(oS.eCache[:,0].A)[0] #转换为array类型
...
def innerL(i,oS):
        ...
        if((oS.labelMat[i]*Ei<-oS.tol) and (oS.alphas[i]<oS.C)) or \
         ((oS.labelMat[i]*Ei>oS.tol) and (oS.alphas[i]>0)):
              ...
              if (oS.labelMat[i] != oS.labelMat[j]):  #第二个alpha选择中的启发式算法
...
def smoP(dataMatIn,classLabels,C,toler,maxIter,kTup=('lin',0)):
        ...
        while(Ite<maxIter) and ((alphaPairsChanged>0) or (entireSet)):
            ...
            if entireSet:
                for i in range(oS.m):
                    alphaPairsChanged+=innerLKer(i,oS)
                    print("fullSet : %d i: %d,pair changed %d"%( Ite,i,alphaPairsChanged))
                Ite+=1
            else:
                nonBoundIs=nonzero((oS.alphas.A>0)*(oS.alphas.A<C))[0]
                for i in nonBoundIs:
                    alphaPairsChanged+=innerLKer(i,oS)
                    print("non_bounds iter:%d i:%d,pairs changed %d"%(Ite,i ,alphaPairsChanged))
                Ite+=1
            if entireSet:
                entireSet=False
